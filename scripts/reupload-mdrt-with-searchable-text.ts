/**
 * Re-upload MDRT Data with Searchable Text
 *
 ...
 *
 * Run: npx tsx scripts/reupload-mdrt-with-searchable-text.ts
 */
...
async function main() {
  ...
  console.log('Re-upload MDRT Data with Searchable Text');
  console.log('='.repeat(80));
  ...
  for (const [namespace, chunks] of chunksByNamespace) {
    ...
    if (vectorsToUpsert.length > 0) {
      ...
      console.log(`   - totalCommission: ${sample.metadata.totalCommission}`);
