/**
 * Re-upload Excel with Fixed doc_type Detection
 *
 * Clears public namespace and re-uploads with correct doc_type
 * Run: npx tsx scripts/reupload-excel-fixed.ts
 */
...
async function main() {
  ...
  console.log('Re-upload with Fixed doc_type Detection');
  console.log('='.repeat(80));
  ...
  console.log(`\n2. Found ${files.length} files in ${DATA_DIR}`);
  ...
  for (const filename of files) {
    ...
    if (ext === '.pdf') {
      ...
      console.log(`  Skipping unsupported: ${ext}`);
      continue;
    }
      ...
      for (let i = 0; i < result.chunks.length; i++) {
        ...
        if ((i + 1) % 50 === 0) {
          process.stdout.write(
            `    Embedded ${i + 1}/${result.chunks.length}\r`
          );
        }
