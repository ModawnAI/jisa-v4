...
import { getServiceClient, getSupabaseClient } from '../utils/supabase.js';
import { getTextFromGPT } from '../services/chat.service.js';
import type {
  KakaoWebhookRequest,
  KakaoResponse,
...
router.post('/chat', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const data: KakaoWebhookRequest = req.body;
    ...
    if (userMessage.toLowerCase() === '/delete' || userMessage.toLowerCase() === '/삭제') {
      ...
      const { data: profile, error: _findError } = await serviceClient
        .from('profiles')
...
관리자에게 새로운 코드를 요청해주세요.`)
      ...
      if (existingProfile) {
        ...
        const existingAuthUser = existingAuthUsers?.users?.find((u: { email?: string }) => u.email === dummyEmail);
      ...
      const { data: newProfile, error: createError } = await serviceClient
        ...
        .update({
          ...
          metadata: {
            ...
            code_purpose: (verificationCode as { purpose?: string }).purpose,
...
• "이번 주 KRS 시험 일정"`;
      ...
      return res.json(createKakaoResponse(welcomeText, quickReplies));
    }
    ...
    return res.json(createKakaoResponse(response));
  } catch (error) {
